  ...
  if(isset($_POST["btn_accion"])){
      ...
      switch($accion){
          ...
          case "Eliminar";
              ...
              if ($txtCant_lc<$cant_capdisp_lc_actual){
                //echo "<script> alert('Hay Disponibilidad de almacenar LC ...'); </script>";
                if($txtCant_ad<$cant_capdisp_ad_actual){
                  //echo "<script> alert('Hay Disponibilidad de almacenar AD...'); </script>";
                  //echo "<script> alert('Procede con el reverso...'); </script>";

                  // Obtengo los saldos del almacén de productos terminados
                  $estatus="A";
                  $nro_empresa=($_POST["txtEmpresa"]);
                  $sentencia=$pdo->prepare("SELECT * FROM `apt` WHERE estatus=:estatus AND nro_empresa=:nro_empresa");
                  $sentencia->bindParam(':estatus',$estatus);
                  $sentencia->bindParam(':nro_empresa',$nro_empresa);
                  $sentencia->execute();
                  $listado_apt=$sentencia->fetchAll(PDO::FETCH_ASSOC);
                  $cant_apt=$sentencia->rowCount();
                  ...
                  $Eliminar=0; //Variable para eliminar o no
                  
                  // Valida la existencia de productos terminados en el APT--------------------------
                  
                  switch($txtNro_queso){
                    case 1; // Queso Duro
                      $Eliminar=1;
                      //echo "<script> alert('Es queso duro...'); </script>";
                      if($txtCant_queso<=$cant_e_qd_actual){
                        //echo "<script> alert('Hay queso duro suficiente...'); </script>";

                      }else{
                        $procesar="ok"; //Muestra Vista normal
                        $error_accion=1; // Valor 0 si todo va normal
                        $mensaje_usuario="La existencia de queso duro no es suficiente"; // Vacío en inicalización
                      }
                    ...
                    case 2;// Queso Mozarella
                      $Eliminar=1;
                      //echo "<script> alert('Es queso mozzarella...'); </script>";
                      if($txtCant_queso<=$cant_e_moz_actual){
                        //echo "<script> alert('Hay queso mozzarella suficiente...'); </script>";

                      }else{
                        $procesar="ok"; //Muestra Vista normal
                        $error_accion=1; // Valor 0 si todo va normal
                        $mensaje_usuario="La existencia de queso mozzarella no es suficiente"; // Vacío en inicalización
                      }
                    ...
                    case 3; // Queso Gouda
                      $Eliminar=1;
                      //echo "<script> alert('Es queso gouda...'); </script>";
                      if($txtCant_queso<=$cant_e_gou_actual){
                        //echo "<script> alert('Hay queso gouda suficiente...'); </script>";
                      }else{
                        $procesar="ok"; //Muestra Vista normal
                        $error_accion=1; // Valor 0 si todo va normal
                        $mensaje_usuario="La existencia de queso gouda no es suficiente"; // Vacío en inicalización
                      }
                    ...
                    case 4; // Queso Dietético
                      $Eliminar=1;
                      //echo "<script> alert('Es queso dietético...'); </script>";
                      if($txtCant_queso<=$cant_e_die_actual){
                        //echo "<script> alert('Hay queso dietético suficiente...'); </script>";

                      }else{
                        $procesar="ok"; //Muestra Vista normal
                        $error_accion=1; // Valor 0 si todo va normal
                        $mensaje_usuario="La existencia de queso dietético no es suficiente"; // Vacío en inicalización
                      }
                    break;
                  }
                  ...
                  $procesar="ok"; //Muestra Vista normal
                  $error_accion=2; // Valor 0 si todo va normal
                  $mensaje_usuario="No hay disponibilidad suficiente en el Almacén de AD "; // Vacío en inicalización
                }

              }else{
                $procesar="ok"; //Muestra Vista normal
                $error_accion=2; // Valor 0 si todo va normal
                $mensaje_usuario="No hay disponibilidad suficiente e en el Almacén de LC"; // Vacío en inicalización
              }
