<?php
  ...
  // Variables de Datos
    $txtFecha=date("d/m/Y");
    $txtCant_lc=number_format(0,2,".","");
    $txtCant_ad=number_format(0,2,".","");
    //print_r($txtFecha);
    ...
    if(isset($_POST["btn_accion"])){
      ...
      switch($accion){
          ...
          case "Calcular";
              ...
              // verifico los montos de almacén
              ...
              // Verifico las existencias de almacén de materia prima
              if( ($txtMonto_total_usb_lc<=0) AND ($txtMonto_total_usb_ad<=0)){
                $procesar="ok"; //Muestra Vista normal
                $error_accion=2; // Valor 0 si todo va normal
                $mensaje_usuario="Los campos deben ser mayor a cero (0)"; // Vacío en inicalización
              }else{
                if($txtCant_litros_lc>$cant_existencia_lc_actual) {
                  $procesar="ok"; //Muestra Vista normal
                  $error_accion=2; // Valor 0 si todo va normal
                  $mensaje_usuario="No hay cantidad de LC suficientes para el ajuste"; // Vacío en inicalización
                }else{
                  if($txtCant_litros_ad>$cant_existencia_ad_actual) {
                    $procesar="ok"; //Muestra Vista normal
                    $error_accion=2; // Valor 0 si todo va normal
                    $mensaje_usuario="No hay cantidad de AD suficientes para el ajuste"; // Vacío en inicalización
                  }else{
                    $calcular="SI";
                  }
                }
          ...
          case "Guardar";
              ...
              // verifico los montos de almacén de materia prima ----------------------------------------------
              ...
              if($txtCant_lc>$cant_existencia_lc_actual){
                $procesar="ok"; //Muestra Vista normal
                $error_accion=2; // Valor 0 si todo va normal
                $mensaje_usuario="No hay existencia suficiente de LC en el almacén:".$cant_existencia_lc_actual; // Vacío en inicalización
              }else {
                if($txtCant_ad>$cant_existencia_ad_actual){
                  $procesar="ok"; //Muestra Vista normal
                  $error_accion=2; // Valor 0 si todo va normal
                  $mensaje_usuario="No hay existencia suficiente de LC en el almacén:".$cant_existencia_ad_actual ; // Vacío en inicalización
                }else {
                  $procesar="ok"; //Muestra Vista normal
                  $error_accion=2; // Valor 0 si todo va normal
                  $mensaje_usuario="¡Buenas Noticias! Todo bien por aquí"; // Vacío en inicalización
                ...
                if ($Cant_Registros>=1){ // Tiene movimientos
                  ...
                  foreach($Registro_AMP_CTO as $RegistroActual){
                    ...
                    $monto_cto_ltr_lc=$RegistroActual['monto_cto_ltr_lc'];
                    $monto_cto_ltr_ad=$RegistroActual['monto_cto_ltr_ad'];
                ...
                $txtTipo_mov_lc="A";
                $txtCant_lc=($_POST["txtCant_lc"]);
                $txtMonto_cto_ltr_lc=$monto_cto_ltr_lc;
                ...
                $txtTipo_mov_ad="A";
                $txtCant_ad=($_POST["txtCant_ad"]);
                $txtMonto_cto_ltr_ad=$monto_cto_ltr_ad;
              ...
              // --------------------------------------------------------------------------------------------------
                ...
                $mensaje_usuario=" El ajuste se realizó satisfactoriamente"; // Vacío en inicalización
                }
              }
